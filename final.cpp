...
void maxHeapify(mainData a[], int i, int n)
{
    ...
    if (left <= n && a[left].dmg > a[i].dmg)
        largest = left;
    else
        largest = i;
    if (right <= n && a[right].dmg > a[largest].dmg)
        largest = right;
    if (largest != i)
    {
...
{
    ifstream read("./text/MainInput.txt");
    long long Inp[3];
    if (read.is_open())
    {
        string line;
    ...
    ft(j, 1, Inp[0])
    {
        ...
        {
            if (!tmp)
                cout << "\t- Nhap sat thuong vu khi " << j << ": ";
            else
                cout << "\t- Nhap lai sat thuong vu khi " << j << " (sat thuong la so tu nhien): ";
            tmp = true;
            cin >> dmg;
        } while (dmg <= 0);
...
{
    if (k == 1)
        cout << "------------------Cac thao tac nguoi choi da thuc hien------------------";
    cout << "\n*Luot danh thu " << k << ": " << '\n';
    ...
    while (key == 0)
    {
        ...
        if (key != -2)
        {
            ...
            if (key == 2)
            {
                cout << "------------------------------------------------------------------------\n";
                if (res <= k && d <= 0)
                    cout << "Ket qua chien dau: Chien thang.\n";
                else
                    cout << "Ket qua chien dau: That bai.\n";
            }
        }
        if (key > 0)
        {
